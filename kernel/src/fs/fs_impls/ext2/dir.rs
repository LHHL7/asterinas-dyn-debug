...
// Inline(never) prevents the compiler from duplicating the dyndbg asm
// symbols when Iterator::next() is codegen'd in multiple compilation units.
#[inline(never)]
fn trace_ext2_dir_entry_scan() {
